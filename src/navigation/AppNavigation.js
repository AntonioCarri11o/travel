...
export default function AppNavigation(){
    ...
    useEffect(()=>{const auth=getAuth(); onAuthStateChanged(auth,(user)=>{ 
       ...
       return session?(
        <Tab.Navigator screenOptions={({route})=>({headerShown:false, tabBarActiveTintColor:"#2A44A5", tabBarInactiveTintColor:"#577AFF", tabBarIcon:({color,size})=>showIcons(route,color,size)})}>
        <Tab.Screen
        name="index"
        component={IndexStack}
        options={{ title: "Inicio" }}
